...
class PostController extends Controller
{
    ...
    public function store(Request $request){
        ...
        $thongbao = json_encode([
                ...
                'title' => $request->title,
                'status'=>'success',
                'message' => '<h3>Thành công!</h3><p class="mb-0">challenge <strong>'.$request->title.'</strong> đã được tạo!</p>',
        ]);
    ...
    public function show($id){
        ...
        if($post) {
            ...
            $thongbao = json_encode(['status'=>'danger', 'message' => 'challenge không tồn tại!']);
    ...
    public function update(Request $request, $id)
    {
        ...
        $thongbao = json_encode([
            ...
            'title' => $request->title,
            'status'=>'success',
            'message' => '<h3>Thành công!</h3><p class="mb-0">challenge <strong>'.$request->title.'</strong> đã được Cập nhật!</p>',
        ]);
        return redirect('/quanly/baiviet/')->with('thongbao', $thongbao);
    }
    public function delete($id)
    {
        ...
        if(!$post) {
            return json_encode([
                        'url' => '/quanly/baiviet/',
                        'status'=> 'error',
                        'message' => '<h3>Lỗi!</h3><p class="mb-0">challenge này không tồn tại!</p>',
                    ]);
        } else {
            try{
            ...
            // Chuyển toàn bộ File về challenge 0
            ...
            $thongbao = json_encode([
                        ...
                        'status'=>'success',
                        'message' => '<h3>Thành công!</h3><p class="mb-0">challenge <strong>'.$post->title.'</strong> đã được xóa thành công!</p>',
                    ]);
